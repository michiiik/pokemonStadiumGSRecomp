/*
 * main.cpp â€” PokemonStadiumGSRecomp runner entry point.
 *
...
std::filesystem::path exe_dir() {
    ...
    if (internal_storage != nullptr && internal_storage[0] != '\0') {
        return std::filesystem::path(internal_storage);
    }
...
// ── Temporary Android freeze-state dump (diagnostic) ─────────────────
//
// Serializes the always-on ultramodern rings plus the live guest queue
// and thread structs into freeze_trace.log. Answers, for the boot stall
// at ~5.65s: is the final SP completion sitting in the scheduler's reply
// queue, is the scheduler thread parked on it, and did the scheduler ever
// re-queue that thread to run afterwards? Remove with the other
// freeze-trace diagnostics.
#include <cstring>
...
extern "C" uint32_t ultramodern_external_pending(void);
...
extern "C" void     ultramodern_sched_recent_copy(void* out, size_t cap, size_t* n_written,
...
// Called from the message-queue tripwire the moment the watched OSMesgQueue
// stops looking like a queue, so the corrupting write is captured before
// anything else runs. Diagnostic; remove with the other freeze instrumentation.

extern "C" void psr_dump_rdram_snapshot(const char* name) {
    static std::atomic<bool> done{false};
    bool expected = false;
    if (!done.compare_exchange_strong(expected, true)) {
        return;
    }
    uint8_t* snap = recomp_runtime_get_rdram();
    if (snap == nullptr) {
        return;
    }
    const auto path = pkmnstadium::exe_dir() / name;
    ...
    for (uint32_t base = 0; base < kRdramSize; base += kChunk) {
    ...
    std::fclose(f);
    std::fprintf(stderr, "[clobber] RDRAM snapshot written to %s\n", name);
    std::fflush(stderr);
}

namespace psr_freeze_state {

// These four must match the ring layouts in ultramodern; the size guards
// below reject drift loudly rather than mis-parsing.
struct MesgEvent {
...
inline const char* mesg_op_name(uint8_t op) {
    static const char* const names[11] = {
        "?", "send_game", "send_external", "recv_enter", "recv_block",
        "recv_return_ok", "ext_deq_ok", "ext_deq_full", "do_send_block",
        "ext_deq_drop", "do_send_drop"};
    return op < 11 ? names[op] : "?";
}

inline const char* guest_state_name(uint32_t state) {
    switch (state) {
...
// Guest words are stored byteswapped, so an aligned host u32 load at the
// masked physical offset yields the value the game sees. Same convention
// as debug_server's rdram_peek.
inline bool read_u32(uint8_t* rdram, uint32_t vaddr, uint32_t* out) {
    if (rdram == nullptr || (vaddr & 3u) != 0u) return false;
    const uint32_t paddr = vaddr & 0x1FFFFFFFu;
    if (paddr + 4u > 0x800000u) return false;
    std::memcpy(out, rdram + paddr, 4);
    return true;
}

inline void dump_guest_thread(FILE* f, uint8_t* rdram, const char* label, uint32_t thread) {
    uint32_t next = 0, pri = 0, queue = 0, flags_state = 0, id = 0;
    if (thread == 0 ||
        !read_u32(rdram, thread + 0u,  &next) ||
        !read_u32(rdram, thread + 4u,  &pri) ||
        !read_u32(rdram, thread + 8u,  &queue) ||
        !read_u32(rdram, thread + 16u, &flags_state) ||
        !read_u32(rdram, thread + 20u, &id)) {
        std::fprintf(f, "%s thread=%08X <unreadable>\n", label, thread);
        return;
    }
    // OSThread declares flags before state, so the byteswapped word at +16
    // carries flags in its low half and state in its high half.
    std::fprintf(f, "%s thread=%08X id=%d pri=%d state=%s flags=%04X queue=%08X next=%08X\n",
                 label, thread, (int)id, (int)pri,
                 guest_state_name(flags_state >> 16),
                 (unsigned)(flags_state & 0xFFFFu), queue, next);
}

inline void dump(FILE* f, const char* tag, uint32_t watch_mq) {
    uint8_t* rdram = recomp_runtime_get_rdram();
    std::fprintf(f, "\n=== freeze_state %s watch_mq=%08X rdram=%s ===\n",
                 tag, watch_mq, rdram ? "ok" : "null");
    std::fprintf(f, "external_requeues=%llu external_pending=%u running_queue_head=%08X\n",
                 (unsigned long long)ultramodern_external_requeues(),
                 ultramodern_external_pending(),
                 ultramodern_running_queue_head());

    uint32_t recv = 0, send = 0, valid = 0, first = 0, msg_count = 0, msgbuf = 0;
    if (read_u32(rdram, watch_mq + 0u,  &recv) &&
        read_u32(rdram, watch_mq + 4u,  &send) &&
        read_u32(rdram, watch_mq + 8u,  &valid) &&
        read_u32(rdram, watch_mq + 12u, &first) &&
        read_u32(rdram, watch_mq + 16u, &msg_count) &&
        read_u32(rdram, watch_mq + 20u, &msgbuf)) {
        std::fprintf(f, "mq %08X validCount=%d first=%d msgCount=%d msgbuf=%08X "
                        "blocked_recv=%08X blocked_send=%08X\n",
                     watch_mq, (int)valid, (int)first, (int)msg_count, msgbuf, recv, send);
        for (uint32_t i = 0; i < msg_count && i < 16u; i++) {
            uint32_t slot = 0;
            if (read_u32(rdram, msgbuf + i * 4u, &slot)) {
                std::fprintf(f, "  slot[%u]=%08X\n", i, slot);
            }
        }
        uint32_t chain = recv;
        for (int i = 0; i < 8 && chain != 0; i++) {
            dump_guest_thread(f, rdram, "  recv_blocked", chain);
            uint32_t next = 0;
            if (!read_u32(rdram, chain, &next)) break;
            chain = next;
        }
        chain = send;
        for (int i = 0; i < 8 && chain != 0; i++) {
            dump_guest_thread(f, rdram, "  send_blocked", chain);
            uint32_t next = 0;
            if (!read_u32(rdram, chain, &next)) break;
            chain = next;
        }
    } else {
        std::fprintf(f, "mq %08X <unreadable>\n", watch_mq);
    }

    if (ultramodern_sched_thread_state_size() != sizeof(SchedThreadState)) {
        std::fprintf(f, "--- sched thread states: size mismatch (runtime=%u local=%u) ---\n",
                     (unsigned)ultramodern_sched_thread_state_size(),
                     (unsigned)sizeof(SchedThreadState));
    } else {
        std::vector<SchedThreadState> states(256);
        size_t got = 0;
        ultramodern_sched_thread_states_copy(states.data(), states.size(), &got);
        std::fprintf(f, "--- sched thread states (%u) ---\n", (unsigned)got);
        for (size_t i = 0; i < got; i++) {
            const SchedThreadState& t = states[i];
            std::fprintf(f, "tid=%u thread=%08X pri=%d last_op=%u last_queue=%08X "
                            "last_head=%08X last_ms=%llu last_mq=%08X last_mq_ms=%llu events=%llu\n",
                         t.id, t.thread, (int)t.priority, t.last_op, t.last_queue,
                         t.last_head_after, (unsigned long long)t.last_ms, t.last_mq,
                         (unsigned long long)t.last_mq_ms, (unsigned long long)t.count);
            dump_guest_thread(f, rdram, "  guest", t.thread);
        }
    }

    if (ultramodern_mesg_qstate_size() != sizeof(QState) ||
        ultramodern_mesg_event_size() != sizeof(MesgEvent)) {
        std::fprintf(f, "--- mesg qstates: size mismatch (qstate %u/%u event %u/%u) ---\n",
                     (unsigned)ultramodern_mesg_qstate_size(), (unsigned)sizeof(QState),
                     (unsigned)ultramodern_mesg_event_size(), (unsigned)sizeof(MesgEvent));
    } else {
        std::vector<QState> qs(1024);
        size_t got = 0;
        ultramodern_mesg_qstates_copy(qs.data(), qs.size(), &got);
        std::fprintf(f, "--- mesg queues (%u) ---\n", (unsigned)got);
        for (size_t i = 0; i < got; i++) {
            const QState& q = qs[i];
            if (q.queue == 0 || q.count == 0) continue;
            const MesgEvent& last = q.last[(q.count - 1u) & 63u];
            std::fprintf(f, "mq=%08X events=%u last: seq=%llu ms=%llu op=%s msg=%08X tid=%u "
                            "vb=%u va=%u block=%u gt=%u\n",
                         q.queue, q.count, (unsigned long long)last.seq,
                         (unsigned long long)last.ms, mesg_op_name(last.op), last.msg,
                         last.thread_id, last.valid_before, last.valid_after,
                         last.block, last.game_thread);
        }
        for (size_t i = 0; i < got; i++) {
            const QState& q = qs[i];
            if (q.queue != watch_mq) continue;
            const uint32_t n = q.count < 64u ? q.count : 64u;
            const uint32_t start = q.count - n;
            std::fprintf(f, "--- mq %08X tail (%u of %u) ---\n", q.queue, n, q.count);
            for (uint32_t k = 0; k < n; k++) {
                const MesgEvent& e = q.last[(start + k) & 63u];
                std::fprintf(f, "seq=%llu ms=%llu op=%-14s msg=%08X thread=%08X tid=%u "
                                "vb=%u va=%u block=%u gt=%u\n",
                             (unsigned long long)e.seq, (unsigned long long)e.ms,
                             mesg_op_name(e.op), e.msg, e.thread, e.thread_id,
                             e.valid_before, e.valid_after, e.block, e.game_thread);
            }
        }
    }

    if (ultramodern_sched_event_size() != sizeof(SchedEvent)) {
        std::fprintf(f, "--- sched ring: size mismatch (runtime=%u local=%u) ---\n",
                     (unsigned)ultramodern_sched_event_size(), (unsigned)sizeof(SchedEvent));
    } else {
        std::vector<SchedEvent> events(512);
        size_t got = 0;
        uint64_t widx = 0;
        ultramodern_sched_recent_copy(events.data(), events.size(), &got, &widx);
        std::fprintf(f, "--- sched ring tail (write_idx=%llu, %u shown) ---\n",
                     (unsigned long long)widx, (unsigned)got);
        for (size_t i = 0; i < got; i++) {
            const SchedEvent& e = events[i];
            std::fprintf(f, "seq=%llu ms=%llu op=%u queue=%08X thread=%08X tid=%u pri=%d "
                            "cur=%08X cur_tid=%u head_after=%08X next_after=%08X\n",
                         (unsigned long long)e.seq, (unsigned long long)e.ms, e.op,
                         e.queue, e.thread, e.thread_id, (int)e.priority,
                         e.current_thread, e.current_thread_id, e.head_after, e.next_after);
        }
    }
    std::fflush(f);
}
...
#if defined(__ANDROID__)
extern "C" void rt64_set_android_surface_ready(int ready);
extern "C" void psr_android_battle_companion_update(void);

static std::atomic<uint16_t> g_android_touch_buttons{0};
static std::atomic<float> g_android_touch_x{0.0f};
static std::atomic<float> g_android_touch_y{0.0f};

extern "C" JNIEXPORT void JNICALL
Java_com_pokemonstadiumrecomp_probe_MainActivity_nativeSetAndroidSurfaceReady(
        JNIEnv*, jclass, jboolean ready) {
    rt64_set_android_surface_ready(ready == JNI_TRUE ? 1 : 0);
}

extern "C" JNIEXPORT void JNICALL
Java_com_pokemonstadiumrecomp_probe_MainActivity_nativeSetTouchControllerState(
        JNIEnv*, jclass, jint buttons, jfloat stick_x, jfloat stick_y) {
    const uint16_t button_mask = static_cast<uint16_t>(buttons);
    const uint16_t previous_buttons =
        g_android_touch_buttons.exchange(button_mask, std::memory_order_acq_rel);
    g_android_touch_x.store(stick_x, std::memory_order_release);
    g_android_touch_y.store(stick_y, std::memory_order_release);
    if (button_mask != previous_buttons) {
        __android_log_print(ANDROID_LOG_INFO, "PSR-Input", "touch buttons=0x%04X", button_mask);
    }
}
#endif
...
// Write-protect one never-used RDRAM page to catch the global 0xFFFF sweep.
// Guest code that legitimately writes here does not exist (the page is far
// above the heap and stays zero in boot snapshots), so any fault is the
// corrupting writer and the tombstone backtrace identifies it.
extern "C" void psr_arm_rdram_watchpage(void) {
    uint8_t* base = recomp_runtime_get_rdram();
    if (base == nullptr) {
        return;
    }
    // 0x80400000 -> rdram + 0x400000. Page-align defensively.
    uint8_t* page = base + 0x400000u;
    const uintptr_t aligned = (uintptr_t)page & ~(uintptr_t)0xFFF;
#if defined(_WIN32)
    DWORD old_protection = 0;
    if (VirtualProtect((void*)aligned, 0x1000, PAGE_READONLY, &old_protection) == 0) {
        std::fprintf(stderr, "[watchpage] VirtualProtect failed: %lu\n",
                     static_cast<unsigned long>(GetLastError()));
    } else {
#else
    if (mprotect((void*)aligned, 0x1000, PROT_READ) != 0) {
        std::fprintf(stderr, "[watchpage] mprotect failed: %s\n", std::strerror(errno));
    } else {
#endif
        std::fprintf(stderr, "[watchpage] 0x80400000 is now read-only; "
                             "the next writer there will fault\n");
    }
    std::fflush(stderr);
}

extern "C" void psr_dump_trace_snapshot(const char* name) {
    static std::atomic<bool> done{false};
    bool expected = false;
    if (!done.compare_exchange_strong(expected, true)) {
        return;
    ...
    const uint64_t write_idx = recomp_ultra_trace_write_idx();
    const uint32_t capacity = recomp_ultra_trace_capacity();
    const uint64_t count = std::min<uint64_t>(write_idx, capacity);
    const uint64_t start = write_idx - count;
    std::fprintf(f, "write_idx=%llu capacity=%u start=%llu\n",
            static_cast<unsigned long long>(write_idx), capacity,
            static_cast<unsigned long long>(start));
    for (uint64_t index = start; index < write_idx; ++index) {
        recomp_ultra_trace_event event{};
        if (!recomp_ultra_trace_get(index, &event)) continue;
        std::fprintf(f,
                "%llu %llu tid=%u thread=%08X %-39s pc=%08X "
                "a0=%08X a1=%08X a2=%08X a3=%08X\n",
                static_cast<unsigned long long>(event.seq),
                static_cast<unsigned long long>(event.ms),
                event.thread_id, event.thread, event.name, event.pc,
                event.a0, event.a1, event.a2, event.a3);
    }
    std::fclose(f);
    std::fprintf(stderr, "[clobber] trace snapshot written to %s\n", name);
    std::fflush(stderr);
}
...
extern RspUcodeFunc aspMain_ps2;
...
static void write_rdram_be_word(uint8_t* rdram, uint32_t vaddr, uint32_t value) {
    const uint32_t paddr = vaddr & 0x1FFFFFFFu;
    rdram[(paddr + 0) ^ 3] = static_cast<uint8_t>(value >> 24);
    rdram[(paddr + 1) ^ 3] = static_cast<uint8_t>(value >> 16);
    rdram[(paddr + 2) ^ 3] = static_cast<uint8_t>(value >> 8);
    rdram[(paddr + 3) ^ 3] = static_cast<uint8_t>(value);
}
...
// Stadium 2 does not ship Stadium 1's audio microcode: its ucode text lives at
// ROM 0x1060 and its ucode_data header (0000 0001 0002 FFFF 0020 0800 7FFF
// 4000 ...) does not match Stadium 1's aspMain dispatch table, which appears
// nowhere in the Stadium 2 ROM. Running Stadium 1's recompiled aspMain against
// Stadium 2 command lists makes its handlers compute RDRAM addresses from the
// wrong ABI and write over unrelated memory. Skipping the task leaves audio
// silent but stops the corruption; replace with a real recompilation of the
// Stadium 2 microcode.
static RspExitReason aspMain_skip(uint8_t* rdram, uint32_t ucode_addr) {
    (void)rdram;
    (void)ucode_addr;
...
// RT64's MetalSwapChain takes its CA::MetalLayer straight from
// WindowHandle::view and never re-derives it. Passing nullptr there left
// layer == nullptr, so MetalSwapChain::isEmpty() was always true and
// PresentQueue::threadLoop skipped EVERY present -- a black window in both
// windowed and fullscreen, while the game rendered correctly into RDRAM.
...
#if defined(__ANDROID__)
    // Keep Android in a stable landscape/fullscreen surface while RT64 creates
    // its Vulkan swapchain. A resizable window can trigger a surface rebuild
    // between SDL_CreateWindow and vkCreateSwapchainKHR on Android 16.
...
#elif defined(__ANDROID__)
    return ultramodern::renderer::WindowHandle{ g_window };
#elif defined(__linux__)
    return ultramodern::renderer::WindowHandle{ wmInfo.info.x11.window };
#elif defined(__APPLE__)
    g_metal_view = SDL_Metal_CreateView(g_window);
    if (g_metal_view == nullptr) {
        fprintf(stderr, "SDL_Metal_CreateView failed: %s\n", SDL_GetError());
        std::exit(EXIT_FAILURE);
    }
    void* metal_layer = SDL_Metal_GetLayer(g_metal_view);
    if (metal_layer == nullptr) {
        fprintf(stderr, "SDL_Metal_GetLayer failed\n");
        std::exit(EXIT_FAILURE);
    }
    return ultramodern::renderer::WindowHandle{ wmInfo.info.cocoa.window, metal_layer };
#endif
}
...
float apply_stick_deadzone(int32_t value, int32_t deadzone) {
    ...
    const int32_t max_value = 32767;
    const float scaled = static_cast<float>(magnitude - deadzone) /
                         static_cast<float>(max_value - deadzone);
...
static void ensure_pad_open() {
    ...
    // plugged in.
    //
    // osContInit runs on a game thread (a background host thread, not
    // the process's actual main thread). On macOS, SDL_PumpEvents pumps
    // Cocoa's NSApplication event queue, and AppKit hard-crashes
    // (NSInternalInconsistencyException, "nextEventMatchingMask should
    // only be called from the Main Thread!") if that happens off the
    // main thread. Skip the pump when called from a non-main thread —
    // controller detection just falls back to the next call from a
    // thread that IS the main thread (get_n64_input's poll_inputs, or a
    // later get_connected_device_info retry) instead of crashing.
#if defined(__APPLE__) && !defined(__ANDROID__)
...
#if defined(__ANDROID__)
    if (primary_port) {
        *buttons_out |= g_android_touch_buttons.load(std::memory_order_acquire);
        const float touch_x = g_android_touch_x.load(std::memory_order_acquire);
        const float touch_y = g_android_touch_y.load(std::memory_order_acquire);
...
static void error_message_box(const char* msg) {
    ...
    fflush(stderr);
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "PokemonStadiumGSRecomp", msg, nullptr);
}
...
int main(int argc, char** argv) {
#if defined(__ANDROID__)
    __android_log_print(ANDROID_LOG_INFO, "PSR", "main() entered argc=%d", argc);
#endif
...
#ifdef __ANDROID__
    // MainActivity copies baserom.z64 and the runtime assets from the APK into
    // SDL's internal-storage directory before SDLActivity starts this thread.
    // Make relative config and asset paths resolve there as well.
    {
        std::error_code cwd_error;
        std::filesystem::current_path(pkmnstadium::exe_dir(), cwd_error);
        if (cwd_error) {
            std::fprintf(stderr, "[PSR] Android current_path failed: %s\n",
    ...
    const bool autoboot =
        ...
        true;
#else
        std::getenv("PSR_AUTOBOOT") != nullptr;
#endif
    if (!autoboot) {
    ...
    recomp::register_config_path(pkmnstadium::exe_dir());
...
#ifdef __ANDROID__
        gconf.hpfb_option = ultramodern::renderer::HighPrecisionFramebuffer::Off;
#endif
    ...
    game.rom_hash               = 0x13DE5F210AA8D502ULL;  // XXH3_64bits of Stadium 2 US baserom.z64
                                                         // computed by tools/compute_rom_hash.cpp
    game.internal_name          = "POKEMON STADIUM 2";
    game.game_id                = u8"pokestadium2.us.1.0";
    game.mod_game_id            = "pokestadium2";
...
#if 0 // Stadium 1 address-specific initialization; not valid for Stadium 2.
    ...
    game.on_init_callback = [](uint8_t* rdram, recomp_context* /*ctx*/) {
        ...
        write_rdram_be_word(rdram, 0x80068B90u, 1);
        std::fprintf(stderr,
            "[PSR] on_init: forced gExpansionRAMStart=1 "
            "(POOL_END_6MB path active)\n");

        const char* boot_mode = std::getenv("PSR_BOOT_MODE");
        if (boot_mode != nullptr && std::strcmp(boot_mode, "battle_now_1p") == 0) {
            // Stadium's own state dispatcher performs the complete Battle Now
            // setup from here; only the logo/title/menu states are skipped.
            write_rdram_be_word(rdram, 0x80075668u, 0x28u);
            std::fprintf(stderr,
                "[PSR] on_init: debug boot mode battle_now_1p "
                "(gCurrentGameState=0x28)\n");
        }
        std::fflush(stderr);
    };
#endif
    ...
    game.on_init_callback = [](uint8_t* /*rdram*/, recomp_context* /*ctx*/) {
        pokestadium::register_runtime_trampolines();
    };
        ...
        while (true) {
            ...
            if (!show_picker(rom_path)) {
                ...
                MessageBoxA(NULL,
                    ...
                    "PokemonStadiumGSRecomp needs a legal copy of Pokemon Stadium 2 (US v1.0) "
                    "to run. Launch the runner again and pick the .z64 file when prompted.",
                    "PokemonStadiumGSRecomp", MB_ICONINFORMATION | MB_OK);
#endif
    ...
    std::thread([game_id = game.game_id]() {
        ...
        std::thread([]() {
        // Temporary Android boot-stall diagnostic. The framework already
        // records this ring continuously; persist one bounded snapshot so it
        // can be read through run-as on devices where debuggerd needs root.
        std::this_thread::sleep_for(std::chrono::seconds(4));
      for (int dump_round = 0; dump_round < 14; dump_round++) {
        const auto trace_path = pkmnstadium::exe_dir() / "freeze_trace.log";
        if (FILE* trace = std::fopen(trace_path.string().c_str(), "w")) {
            const uint64_t write_idx = recomp_ultra_trace_write_idx();
            const uint32_t capacity = recomp_ultra_trace_capacity();
            const uint64_t count = std::min<uint64_t>(write_idx, capacity);
            const uint64_t start = write_idx - count;
            std::fprintf(trace, "write_idx=%llu capacity=%u start=%llu\n",
                    static_cast<unsigned long long>(write_idx), capacity,
                    static_cast<unsigned long long>(start));
            for (uint64_t index = start; index < write_idx; ++index) {
                recomp_ultra_trace_event event{};
            ...
            // Every ROM->RDRAM PI DMA with its destination, so the
            // fragment-67 overwrite can be attributed to a real transfer
            // rather than inferred from the resulting bytes.
            const auto pi_path = pkmnstadium::exe_dir() / "pi_fills.bin";
            const int64_t pi_fills = recomp_pi_fill_dump(pi_path.string().c_str());
            std::fprintf(trace, "pi_fill_dump=%lld\n", (long long)pi_fills);

            // Whole-RDRAM snapshot so the extent and source of the overwrite
            // can be established offline against the ROM. XOR-3 byte access
            // matches the recompiler's swap convention.
            if (uint8_t* snap_rdram = recomp_runtime_get_rdram()) {
                const auto rdram_path = pkmnstadium::exe_dir() / "rdram.bin";
                if (FILE* rf = std::fopen(rdram_path.string().c_str(), "wb")) {
                    constexpr uint32_t kRdramSize = 0x800000u;
                    constexpr uint32_t kChunk = 0x10000u;
                    std::vector<uint8_t> buf(kChunk);
                    for (uint32_t base = 0; base < kRdramSize; base += kChunk) {
                        for (uint32_t i = 0; i < kChunk; i++) {
                            buf[i] = snap_rdram[(base + i) ^ 3];
                        }
                        std::fwrite(buf.data(), 1, kChunk, rf);
                    }
                    std::fclose(rf);
                    std::fprintf(trace, "rdram_snapshot=ok\n");
                }
            }
            psr_freeze_state::dump(trace, "periodic", 0x800CDA10u);
            std::fclose(trace);
        }
...
#if defined(__ANDROID__)
        psr_android_battle_companion_update();
#endif
    };
...
#if defined(__ANDROID__)
// SDLActivity invokes the exported SDL_main entry point on its SDL thread.
// The desktop build intentionally keeps the real entry point named main().
extern "C" __attribute__((visibility("default"))) int SDL_main(int argc, char** argv) {
#if defined(__ANDROID__)
    __android_log_print(ANDROID_LOG_INFO, "PSR", "SDL_main() entered argc=%d", argc);
    // Android drops stderr, and the runtime reports the reason a fragment
    // fails to bind ([reg-frag], [synth-frag], [pi], ...) only on stderr.
    // Point it at a file the debug loop can pull with run-as. Diagnostic.
    {
        const auto psr_stderr_log = pkmnstadium::exe_dir() / "stderr.log";
        std::freopen(psr_stderr_log.string().c_str(), "w", stderr);
