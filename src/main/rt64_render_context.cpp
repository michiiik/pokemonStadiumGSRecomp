...
#ifdef __ANDROID__
    // RT64's desktop path detector sees Android's HOME as /data, which is not
    // writable by an application. Keep its cache under the app-private files
    // directory exposed by SDL instead.
    appConfig.detectDataPath = false;
    appConfig.dataPath = pkmnstadium::exe_dir() / "rt64";
#endif
...
#ifdef __ANDROID__
    // Option 2: present as soon as rendering completes instead of pacing to
...
void pokestadium::renderer::RT64Context::update_screen() {
    // PSR_VIEW_RDRAM=1 presents the guest RDRAM framebuffer instead of RT64's
    // own high-res render target for the VI origin.
    //
    // Stadium 2 puts content on screen that never passes through the RDP: the
    // title art is produced by a non-graphics RSP task straight into RDRAM (the
    // "other" task type in the progress counters). RT64 correctly FINDS a
    // framebuffer for the VI origin -- fbAddress() subtracts one row, so
    // 0x769F80-0x500 = 0x769A80 and 0x7B4C80-0x280 = 0x7B4A00 both match
    // targets it owns -- but that target was never rendered into, so it
    // presents black while RDRAM holds the complete image.
    //
    // This is RT64's own developer view mode (normally a keyboard shortcut),
    // set here from our glue so lib/rt64 -- which is SHARED with the Stadium 1
    // build -- stays untouched. Costs the high-res render; native res only.
    static const bool force_view_rdram = [] {
        const char* e = std::getenv("PSR_VIEW_RDRAM");
        return e != nullptr && e[0] == '1';
    }();
    if (force_view_rdram && app != nullptr && app->presentQueue != nullptr) {
        app->presentQueue->viewRDRAM = true;
    }
